<template>
  <div class="draw-control">
    ...
    <div v-if="showDrawMenu" class="draw-menu">
      <button
        @click="activateDrawMode('Rectangle')"
        :class="{ active: activeDrawMode === 'Rectangle' }"
      >
      ...
      <button
        @click="activateDrawMode('Polygon')"
        :class="{ active: activeDrawMode === 'Polygon' }"
      >
        <div class="button-content">
...
const emit = defineEmits(['draw-complete', 'draw-mode-activated', 'deactivate-draw-mode'])
...
function activateDrawMode(mode) {
  ...
  drawInteraction.value.on('drawend', (event) => {
    ...
    const coordinates = mode === 'Circle' ? circleToPolygon(geometry) : geometry.getCoordinates()
...
function createBox() {
  return function (coordinates, geometry) {
    ...
    const boxCoordinates = [[start, [start[0], end[1]], end, [end[0], start[1]], start]]
